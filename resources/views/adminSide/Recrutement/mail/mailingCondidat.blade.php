...
@section('content')
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <span id="nom" class="d-none">{{$condidat[0]['nom']}}</span>
              ...
              <div class="card-body">
                <div class="form-group">
                  <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Collaborateur en Copie:</label>
                        <select class="form-control">
                          <option value="regeter">user 1</option>
                          <option value="selectione">user 2</option>
                          <option value="confirmation">user 3</option>
                ...
                <div class="form-group">
                  ...
                  <input class="form-control " name="a" placeholder="To:" value=" {{$condidat[0]['email']}}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea id="compose-textarea" class="form-control mail" style="height: 300px">
                      txt
                    </textarea>
                </div>
                <div class="form-group">
    ...
    $("li").on("click", function() {
     ...
     $(".decision").on("click", function() {
        ...
        if($('.decision :selected').val()== "regeter"){
            $(".mail").html("Bonjour "+nom+" "+prenom+",Nous tenons à vous remercier de la confiance que vous nous témoignez, et de l’intérêt que vous portez aux activités de notre entreprise, en nous proposant votre collaboration pour le poste {TITRE_ANNONCE}.Malheureusement, malgrès tout l’intérêt que présente votre dossier, nous ne pouvons pas y donner une suite favorable.Nous nous permettons cependant de conserver votre dossier et ne manquerons pas de vous contacter si une nouvelle opportunité venait à se présenter.En vous souhaitant de réussir dans votre recherche actuelle.Cordialement;Direction des Ressources Humaines {NOM_ENTREPRISE}");
            $(".objectMail").val($('.decision :selected').text());

        }else if($('.decision :selected').val()== "selectione"){
            $(".mail").html("Bonjour "+nom+" "+prenom+", Nous avons le plaisir de vous annoncer que votre candidature à l'offre {TITRE_ANNONCE} a été sélectionnée. Nous vous invitons à passer un entretien au niveau de notre siège à l'adresse ................., le JJ/MM/AAAA à hh:mm. Merci de venir muni(e) de votre ................. (ex: CV mis à jour / pièce d'identité / Mail imprimé).Cordialement; Direction des Ressources Humaines {NOM_ENTREPRISE}");
            $(".objectMail").val($('.decision :selected').text());

        }else if($('.decision :selected').val()== "confirmation"){
            $(".mail").html("Bonjour {PRENOM_DESTINATAIRE} {NOM_DESTINATAIRE} ,Nous tenons à vous informer que nous avons bien reçu votre candidature à l'offre {TITRE_ANNONCE}.Nous procéderons à l'étude de votre dossier et vous contacterons dans le cas ou votre candidature est retenue, dans le cas contraire à défaut de recevoir un mail de réponse négative de notre part dans un délai dépassant les 8 semaines, merci de comprendre que nous ne sommes pas en mesure d'y donner une suite favorable pour le moment.Cordialement,Direction des Ressources Humaines {NOM_ENTREPRISE}");
            $(".objectMail").val($('.decision :selected').text());

        }else if($('.decision :selected').val()== "confirmAnonyme"){
            $(".mail").html("Bonjour {PRENOM_DESTINATAIRE} {NOM_DESTINATAIRE}, Votre candidature à l'offre {TITRE_ANNONCE} a bien été transmise à l'entreprise qui recrute.L'entreprise prendra contact avec vous dans le cas ou votre candidature correspond au profil recherché. Dans le cas contraire, Emploitic en tant que média n'intervient pas dans le processus de recrutement de l'entreprise.Cordialement,L’équipe Emploitic.com");
